...
import { WEATHER_API_URL,WEATHER_API_KEY } from "./components/servis";
...
export const WeatherProvider = (props) => {
	...
	const getCurrentWeather = async (searchValue) => {
		try {
			let response = await fetch(`${WEATHER_API_URL}/weather?q=${searchValue}&appid=${WEATHER_API_KEY}&units=metric`)
			if (!response.ok) {
